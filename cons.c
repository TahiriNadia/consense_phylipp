...
void initconsnode2(node **p, node **grbg, node *q, long len, long nodei,
                        long *ntips, long *parens, initops whichinit,
                        pointarray treenode, pointarray nodep, Char *str,
                        Char *ch, char ** treesNewick, int nb_arb, int * j)
{
  /* initializes a node */
  long i;
  char c;
...
/*     if (firsttree && prntsets) {
  ...
  case length:
    //processlength(&valyew, &divisor, ch, &minusread, intree, parens);
	(*j)++;
	processlength2(&valyew, &divisor, ch, &minusread, parens, treesNewick, nb_arb, j);
    fracchange = 1.0;
    (*p)->v = valyew / divisor / fracchange;
    break;
  case treewt:
...
void read_groups (pattern_elm ****pattern_array,
        long total_trees, long tip_count, char ** treesNewick)
{
  ...
  grbg = NULL;
  initial = true;
  for(nb_arb = 0 ; nb_arb < total_trees;nb_arb++){
    for (i = 0; i < maxgrp; i++) {
      lengths[i] = -1;
    }
    goteof = false;
    nextnode = 0;
    haslengths = true;
    allocate_nodep(&nodep, &spp,nb_arb,treesNewick);
    assert(spp == tip_count);		
    treeread(&root, treenode, &goteof, &firsttree, nodep,
              &nextnode, &haslengths, &grbg, initconsnode2,true,-1,treesNewick,nb_arb);
    if (!initial) {
      missingname(root);
      reordertips();
    } else {
