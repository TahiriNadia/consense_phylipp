...
void getch3(Char *c, long *parens, char ** treesNewick,int nb_arb,int j)
{ /* get next nonblank character */
  if(j < strlen(treesNewick[nb_arb])){
    (*c) = treesNewick[nb_arb][j];
    if ((*c) == '(')
      (*parens)++;
    if ((*c) == ')')
      (*parens)--;
...
void processlength2(double *valyew, double *divisor, Char *ch,
        boolean *lengthIsNegative, long *parens, char ** treesNewick, int nb_arb, int * j)
{ /* read a branch length from a treefile */
  ...
  *divisor = 1.0;
  (*j)++;
  getch3(ch, parens, treesNewick, nb_arb, (*j));
...
long countcomma(long *comma,int tree_in_size,char ** treesNewick)
{
  ...
  for(tree_in = 0; tree_in < strlen(treesNewick[tree_in_size]);tree_in++){
    //if (treesNewick[i][j] == ';') break;
    if(treesNewick[tree_in_size][tree_in] == ',')  (*comma)++;
    if(treesNewick[tree_in_size][tree_in] == '(')  lparen++;
    if(treesNewick[tree_in_size][tree_in] == '[')  bracket++;
  }
...
void allocate_nodep(pointarray *nodep,long  *precalc_tips,int tree_in_size, char ** treesNewick)
{ /* pre-compute space and allocate memory for nodep */

  long numnodes;      /* returns number commas & (    */
  long numcom = 0;        /* returns number commas */

  numnodes = countcomma(&numcom,tree_in_size,treesNewick) + 1;
...
long take_name_from_tree2 (Char *ch, Char *str, char **treesNewick, int nb_arb, int * j)
{
  /* This loop reads a name from treefile and stores it in *str.
     Returns the length of the name string. str must be at
     least MAXNCH bytes, but no effort is made to null-terminate
  ...
  do {
	...
	(*ch) = treesNewick[nb_arb][(*j)];
    if (*ch == '\n')
      *ch = ' ';
  } while ( strchr(":,)[;", *ch) == NULL );

    return name_length;
}  /* take_name_from_tree2 */
...
void addelement(node **p, node *q, Char *ch, long *parens,
        pointarray treenode, boolean *goteof, boolean *first, pointarray nodep,
        long *nextnode, long *ntips, boolean *haslengths, node **grbg,
        initptr2 initnode, boolean unifok, long maxnodes,char ** treesNewick,int nb_arb,int *j)
{
  ...
  if ((*ch) == '(') {
    ...
    (*initnode)(p, grbg, q, len, nodei, ntips,
                  parens, bottom, treenode, nodep, str, ch, treesNewick, nb_arb, j);

    //printf("Ch après   = %c\n",ch);
    //printf("parens après = %ld\n",parens);
    pfirst      = (*p);
    notlast = true;
    while (notlast) {          /* loop through immediate descendants */
      ...
      (*initnode)(&(*p)->next, grbg, q,
                   ...
                   nodep, str, ch, treesNewick, nb_arb, j);
                       /* ... doing what is done before each */
      r = (*p)->next;
  
      (*j)++;  
      getch3(ch, parens,treesNewick,nb_arb,(*j));      /* look for next character */
	  //getch(ch, parens,treefile);
	  //printf("1-Ch après l'appel = %c %d \n", *(ch), (*j));
       /* handle blank names */
      if((*ch) == ',' || (*ch) == ':'){
        //ungetc((*ch), treefile);
		(*j)--;
      ...
      addelement(&(*p)->next->back, (*p)->next, ch, parens,
        treenode, goteof, first, nodep, nextnode, ntips,
        haslengths, grbg, initnode, unifok, maxnodes,treesNewick,nb_arb,j);

      (*initnode)(&r, grbg, q, len, nodei, ntips,
                    parens, hslength, treenode, nodep, str, ch, treesNewick, nb_arb, j);
                           /* do what is done after each about length */
      pfirst->numdesc++;               /* increment number of descendants */
      *p = r;                         /* make r point back to p */
      /*printf("Avant le if ch =  %c\n",*(ch));
      printf("Avant le if tree_in_size = %d\n",tree_in_size);*/
      if ((*ch) == ')') {
        notlast = false;
        do {
			(*j)++;
			getch3(ch, parens,treesNewick,nb_arb,(*j));
			//getch(ch, parens,treefile);
            //printf("2-Ch après l'appel = %c %d \n", *(ch), (*j));            
            //printf("Ch après l'appel = %c\n", *(ch));

            //printf("ch dans le do = %c\n",*(ch));
          //  printf("tree_in_size dans le do = %d\n",tree_in_size);
        } while ((*ch) != ',' && (*ch) != ')' &&
           (*ch) != '[' && (*ch) != ';' && (*ch) != ':');
      }
    }
	...
	//(*j)++;
    len = take_name_from_tree2 (ch, str, treesNewick, nb_arb, j);  /* get the name */
	//len = take_name_from_tree (ch, str, treefile);
	
    if ((*ch) == ')')
      (*parens)--;         /* decrement count of open parentheses */
    (*initnode)(p, grbg, q, len, nodei, ntips,
                  parens, tip, treenode, nodep, str, ch, treesNewick, nb_arb, j);
                              /* do what needs to be done at a tip */
  } else{
  	  (*j)++;
      getch3(ch, parens,treesNewick,nb_arb,(*j));
	  //getch(ch, parens,treefile);
      //printf("3-Ch après l'appel = %c %d \n", *(ch), (*j));
 }
  if (q != NULL)
    hookup(q, (*p));                    /* now hook up */
  (*initnode)(p, grbg, q, len, nodei, ntips,
                parens, iter, treenode, nodep, str, ch, treesNewick, nb_arb, j);
                              /* do what needs to be done to variable iter */
  if ((*ch) == ':')
    (*initnode)(p, grbg, q, len, nodei, ntips,
                  parens, length, treenode, nodep, str, ch, treesNewick, nb_arb, j);
                                   /* do what needs to be done with length */
  else if ((*ch) != ';' && (*ch) != '[')
    (*initnode)(p, grbg, q, len, nodei, ntips,
                  parens, hsnolength, treenode, nodep, str, ch, treesNewick, nb_arb, j);
                             /* ... or what needs to be done when no length */
  if ((*ch) == '[')
    (*initnode)(p, grbg, q, len, nodei, ntips,
                  parens, treewt, treenode, nodep, str, ch, treesNewick, nb_arb, j);
                             /* ... for processing a tree weight */
  else if ((*ch) == ';')     /* ... and at end of tree */
    (*initnode)(p, grbg, q, len, nodei, ntips,
                  parens, unittrwt, treenode, nodep, str, ch, treesNewick, nb_arb, j);
				  
}  /* addelement */


void treeread (node **root, pointarray treenode,
        boolean *goteof, boolean *first, pointarray nodep,
        long *nextnode, boolean *haslengths, node **grbg, initptr2 initnode,
        boolean unifok, long maxnodes,char **treesNewick,int nb_arb)
{
  ...
  if(j>=strlen(treesNewick[nb_arb])) return;
  
  getch3(&ch,&parens,treesNewick,nb_arb,j);
  //getch(&ch, &parens,treefile);
  //printf("00-Ch après l'appel = %c \n", ch);
  while (ch != '(') {
    /* Eat everything in the file (i.e. digits, tabs) until you
       ...
       getch3(&ch,&parens,treesNewick,nb_arb,j);
	   //getch(&ch, &parens,treefile);
	   //printf("0-Ch après l'appel = %c \n", ch);
  }
  if (haslengths != NULL)
    *haslengths = true;
  addelement(root, NULL, &ch, &parens, 
         treenode, goteof, first, nodep, nextnode, &ntips,
         haslengths, grbg, initnode, unifok, maxnodes,treesNewick,nb_arb,&j);
  ...
  while (j<strlen(treesNewick[nb_arb]));
  //while (eoln(treefile) && !eoff(treefile));
